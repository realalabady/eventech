...
/**
 * Lazy boundary for OrganizationSettings.
 *
 ...
 */
const Lazy = dynamic(
  () => import("./organization-settings").then((m) => m.OrganizationSettings),
  { loading: () => <Fallback /> },
);

function Fallback() {
  const t = useTranslations("common");
  return <FormSkeleton label={t("loading")} fields={4} />;
}

export function LazyOrganizationSettings(props: React.ComponentProps<typeof Lazy>) {
  return <Lazy {...props} />;
}
